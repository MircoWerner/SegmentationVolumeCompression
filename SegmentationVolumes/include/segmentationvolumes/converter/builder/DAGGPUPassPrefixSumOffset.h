...
namespace raven {
    class DAGGPUPassPrefixSumOffset final : public PassShaderCompute {
    public:
        explicit DAGGPUPassPrefixSumOffset(GPUContext *gpuContext) : PassShaderCompute(gpuContext, "DAGGPUPassPrefixSumOffset") {
        }
    ...
    protected:
        std::vector<std::shared_ptr<Shader>> createShaders() override {
            ...
            return {std::make_shared<Shader>(m_gpuContext, Paths::m_resourceDirectoryPath + "/shaders/builder", "dag_3_1_prefix_sum_offset.comp", vk::ShaderStageFlagBits::eCompute, workGroupSize, specializationMapEntries, specializationData)};
        }
