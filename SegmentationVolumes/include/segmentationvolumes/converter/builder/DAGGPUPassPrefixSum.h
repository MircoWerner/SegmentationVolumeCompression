...
namespace raven {
    class DAGGPUPassPrefixSum final : public PassShaderCompute {
    public:
        explicit DAGGPUPassPrefixSum(GPUContext *gpuContext) : PassShaderCompute(gpuContext, "DAGGPUPassPrefixSum") {
        }
    ...
    protected:
        std::vector<std::shared_ptr<Shader>> createShaders() override {
            ...
            return {std::make_shared<Shader>(m_gpuContext, Paths::m_resourceDirectoryPath + "/shaders/builder", "dag_3_0_prefix_sum.comp", vk::ShaderStageFlagBits::eCompute, workGroupSize, specializationMapEntries, specializationData)};
        }
