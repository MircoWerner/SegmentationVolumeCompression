...
namespace raven {
    class DAGGPUPassUnique final : public PassShaderCompute {
    public:
        explicit DAGGPUPassUnique(GPUContext *gpuContext) : PassShaderCompute(gpuContext, "DAGGPUPassUnique") {
        }
    ...
    protected:
        std::vector<std::shared_ptr<Shader>> createShaders() override {
            ...
            return {std::make_shared<Shader>(m_gpuContext, Paths::m_resourceDirectoryPath + "/shaders/builder", "dag_2_unique.comp", vk::ShaderStageFlagBits::eCompute, workGroupSize, specializationMapEntries, specializationData)};
        }
