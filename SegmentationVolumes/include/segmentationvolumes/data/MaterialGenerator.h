...
namespace raven {
    ...
    public:
        ...
        MaterialGenerator(std::function<glm::vec3(Type id)> propertyGeneratorFunction, const uint32_t numProperties) : m_numProperties(numProperties), m_propertyGeneratorFunction(std::move(propertyGeneratorFunction)) {}

        glm::vec3 operator()(const Type type) const {
            return m_propertyGeneratorFunction(type);
        }

        static std::function<glm::vec3(Type id)> default400PropertyGeneratorFunction() {
            return [](const Type id) -> glm::vec3 {
                return {m_COOL_WARM[id * 3 + 0], m_COOL_WARM[id * 3 + 1], m_COOL_WARM[id * 3 + 2]};
            };
        }

        static std::function<glm::vec3(Type id)> default1000PropertyGeneratorFunction() {
            return [](const Type id) -> glm::vec3 {
                glm::vec3 diffuse;
                switch (id) {
                    case 0:
                        ...
                        break;
                    case 1:
                        diffuse = glm::vec3(145, 0, 0) / 255.f;
                        break;
                    case 21:
                        diffuse = glm::vec3(235, 237, 182) / 255.f;
                        break;
                    case 27:
                        diffuse = glm::vec3(70, 76, 90) / 255.f;
                        break;
                    default:
                        const auto idf = static_cast<float>(id);
                        diffuse = glm::vec3(random(glm::vec2(idf, idf)), random(glm::vec2(idf, 2 * idf)), random(glm::vec2(2 * idf, idf)));
                        break;
                }
                return diffuse;
            };
        ...
        static std::function<glm::vec3(Type id)> defaultMouseCortexAxonPropertyGeneratorFunction() {
            return [](const Type id) -> glm::vec3 {
                const auto idf = static_cast<float>(id);
                return {0.5f * random(glm::vec2(idf, idf)) + 0.5f, 0.5f - 0.5f * random(glm::vec2(idf, 2 * idf)), 0};
            };
        }

        static std::function<glm::vec3(Type id)> defaultMouseCortexDendritePropertyGeneratorFunction() {
            return [](const Type id) -> glm::vec3 {
                const auto idf = static_cast<float>(id);
                return {0, 0.5f - 0.5f * random(glm::vec2(2 * idf, idf)), 0.5f * random(glm::vec2(idf, idf)) + 0.5f};
            };
        }

        static std::function<glm::vec3(Type id)> defaultMouseCortexNeuronPropertyGeneratorFunction() {
            return [](const Type id) -> glm::vec3 {
                return glm::vec3(m_DISTINCT_COLORS[id * 3 + 0], m_DISTINCT_COLORS[id * 3 + 1], m_DISTINCT_COLORS[id * 3 + 2]) / 255.f;
            };
        }

    private:
        std::function<glm::vec3(Type id)> m_propertyGeneratorFunction;

        static double random(const glm::vec2 st) {
            // https://thebookofshaders.com/10/
            return glm::fract(glm::sin(glm::dot(st, glm::vec2(12.9898, 78.233))) * 43758.5453123);
        }

        // https://www.kennethmoreland.com/color-advice/
        constexpr static float m_COOL_WARM[256 * 3] = {0.22999950386952345, 0.2989989340493756, 0.754000138575591,
        ...
        constexpr static uint8_t m_DISTINCT_COLORS[96 * 3] = {105, 105, 105,
                                                                    128, 128, 128,
                                                                    169, 169, 169,
                                                                    220, 220, 220,
                                                                    47, 79, 79,
